...
export function ServicesPricingView() {
  ...
  const saveStatusLine = () => {
    if (loadError) {
      return `Cloud sync paused: ${loadError} Refresh before editing to protect saved pricing.`;
    }
          ...
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              ...
              <span
                className={cn(
                  "text-sm font-medium",
                  saveError || loadError ? "text-amber-400" : "text-emerald-400",
                )}
              >
                {loadError ? "Sync paused" : saveError ? "Save issue" : saving ? "Saving…" : "Cloud sync"}
              </span>
            </div>
            {saveError && !loadError ? (
              <button
